...
namespace Catalog.API.Data
{
    public class CatalogInitailData : IInitialData
    {
        ...
        {
            using var seccion = store.LightweightSession();

            if (await seccion.Query<Product>().AnyAsync())
            {
                return;
            }
            // Marten UPSERT will cater for existing records
            seccion.Store<Product>(GetPreconfigureProducts());
            await seccion.SaveChangesAsync();
        }
